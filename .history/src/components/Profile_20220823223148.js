...
export default function Profile() {
  ...
  const ava = document.getElementById('halo')
  ava?.addEventListener('mouseover', () => console.log('halo'))
  return (
    <>
      <Box className={classes.container}>
        <Box className={`${classes.card} shadow`}>
          <Box display="inline-flex" width='100%' justifyContent={'center'}>
            <Avatar id="halosx={{ width: '56px', height: '56px' }} src={image ? image : user.picture}></Avatar>
          </Box>
          <Box>
            <Box pb={2}>
