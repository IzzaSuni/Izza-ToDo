...
const uploadImages = ({ images, name }) => {
  const imagesRef = ref(storage, `images/${ name } profile pict ` );
  // uploadBytes(imagesRef, images).then((e) => console.log(e));
  // // list(ref(storage,'images/'),{name:'Izza profile pict'}).then(e=>console.log(e))
  // storage.ref('images/undefined profile pict.png')
  const a= getDownloadURL(ref(storage,'images/undefined profile pict.png')).then(e=>console.log(e))
  console.log(a)
};
...
const GetUser = (name) => {
  ...
  const getData = async () => {
    try {
      await getDocs(colRef).then((e) => {
        return e.docs.forEach((ev) => {
          data = ev.data().username === name;
        });
      });
      return data;
    } catch (e) {
